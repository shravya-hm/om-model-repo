package Nanofiltration_CP
 ...
 record set_flow_parameters
  ...
  parameter Real k_w = 9.3;   //water permeability through the membrane in L/hm^2
 ...
 //c_f is the only variation from one membrane module to the next
  ...
  Real B;
    ...
    parameter Real c_raw = 100 "Concentration of contaminant / mg/m3";
    Real Q_raw "raw water flowrate calculated based on Y / m3/s " ;
    Real Q_p;
    Real J_p;
 ...
 P_permeate = In_P1.P + In_P2.P + In_P3.P + In_P4.P;
...
//Out_cyc.Q = -abs(rec);
  ...
  R_raw_overall = 1 - (Pn.c_permeate/RW.c_raw);
